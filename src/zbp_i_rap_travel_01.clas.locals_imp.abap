...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD get_instance_features.
    ...
    READ ENTITIES OF zi_rap_travel_01 IN LOCAL MODE
      ENTITY Travel
  ...
  METHOD acceptTravel.
    ...
    MODIFY ENTITIES OF zi_rap_travel_01 IN LOCAL MODE
      ENTITY Travel
         UPDATE
           FIELDS ( TravelStatus )
  ...
  METHOD validateAgency.
    DATA it_agencies TYPE SORTED TABLE OF /dmo/agency WITH UNIQUE KEY agency_id.

    "Read relevant agency ID
    READ ENTITIES OF zi_rap_travel_01 IN LOCAL MODE
      ENTITY Travel
          FIELDS ( AgencyID ) WITH CORRESPONDING #( keys )
          RESULT DATA(travels).

*  LOL, NOT ANYMORE LIKE THIS, WTF.
*   LOOP AT travels INTO DATA(wa_travels).
*   ENDLOOP.
*
    it_agencies = CORRESPONDING #( travels DISCARDING DUPLICATES MAPPING agency_id = AgencyID EXCEPT * ).

    IF it_agencies IS NOT INITIAL.
      "Check if agencyID exist
      SELECT FROM /dmo/agency FIELDS agency_id
        FOR ALL ENTRIES IN @it_agencies
         WHERE agency_id = @it_agencies-agency_id
            INTO TABLE @DATA(it_agencies_db).
    ENDIF.

    "Raise msg for non existing or initial agencyID
    LOOP AT travels INTO DATA(lw_travel).
      "Clear state message that may exist
      APPEND VALUE #( %tky = lw_travel-%tky
                      %state_area = 'VALIDATE_AGENCY')
      TO reported-travel.

      IF lw_travel-AgencyID IS INITIAL OR NOT line_exists( it_agencies_db[ agency_id = lw_travel-AgencyID ] ).
        APPEND VALUE #( %tky = lw_travel-%tky ) TO failed-travel.

        APPEND VALUE #( %tky = lw_travel-%tky
                        %state_area = 'VALIDATE_AGENCY'
                        %msg        = NEW zcm_rap_01(
                                       severity = if_abap_behv_message=>severity-error
                                       textid   =  zcm_rap_01=>agency_unknown
  ...
  METHOD validateCustomer.
    ...
    DATA: it_customer TYPE SORTED TABLE OF /dmo/customer WITH UNIQUE KEY customer_id.
    ...
    it_customer = CORRESPONDING #( it_travel DISCARDING DUPLICATES MAPPING customer_id = CustomerID EXCEPT * ).
    DELETE it_customer WHERE customer_id IS INITIAL.

    IF it_customer IS NOT INITIAL.
      SELECT FROM /dmo/customer FIELDS customer_id
       FOR ALL ENTRIES IN @it_customer
           WHERE customer_id = @it_Customer-customer_id
              INTO TABLE @DATA(it_customer_db).
    ENDIF.

    LOOP AT it_Travel INTO DATA(lw_travel).
      APPEND VALUE #( %tky = lw_travel-%tky
                      %state_area = 'VALIDATE_CUSTOMER' )
                 TO reported-travel.
      IF lw_travel-CustomerID IS INITIAL OR NOT line_exists( it_customer_db[ customer_id = lw_travel-CustomerID  ] ).
        APPEND VALUE #( %tky = lw_travel-%tky ) TO failed-travel.
        ...
        APPEND VALUE #( %tky = lw_travel-%tky
                        ...
                        %msg        = NEW zcm_rap_01(
                               severity   = if_abap_behv_message=>severity-error
                               textid     = zcm_rap_01=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_01(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_01=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_01(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_01=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
...
*   For the Travel Status we need the before-image from the database. We perform this for active (is_draft=00) as well as for drafts (is_draft=01) as we can't distinguish between edit or new drafts
      ...
      IF is_update_requested = abap_true.
        ...
        IF has_before_image = abap_true.
          ...
          IF update_granted = abap_false.
            APPEND VALUE #( %tky        = travel-%tky
                            %msg        = NEW zcm_rap_01( severity = if_abap_behv_message=>severity-error
                                                            textid   = zcm_rap_01=>unauthorized )
                          ) TO reported-travel.
          ENDIF.
  ...
  METHOD is_create_granted.
    AUTHORITY-CHECK OBJECT 'ZOSTAT01'
         ID 'ZOSTAT01' DUMMY
         ID 'ACTVT' FIELD '01'.
  ...
  METHOD is_delete_granted.
    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZOSTAT01'
        ID 'ZOSTAT01' FIELD overall_status
        ID 'ACTVT' FIELD '06'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZOSTAT01'
        ID 'ZOSTAT01' DUMMY
